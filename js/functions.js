           ...
           if (response != 'error')
            {
                ...
                $('.bodyModal').html('<form action=""method="post"name="form_del_product" id="form_del_product" onsubmit="event.preventDefault(); delProduct();">'+
                                   ...
                                   '<a href="#" class="btn_ok" onclick="coloseModal();">Cerrar</a>'+
                                     '</form>');                    
            }
...
$('.btn_new_cliente').click(function(e){
    ...
    $('#tip_cliente').removeAttr('disabled');
    $('#gen_cliente').removeAttr('disabled');
        ...
        $.ajax({
            ...
            success: function(response)
            {
               ...
               if (response == 0) {
                    ...
                    $('#tip_cliente').val(data.documento);
                    $('#gen_cliente').val(data.genero);
                    ...
                    $('#tip_cliente').attr('disabled','disabled');
                    $('#gen_cliente').attr('disabled','disabled');
...
//Agregar producto al detalle_temp
              ...
              if (response != 'error')
               {
                   ...
                   $('txt_cant_producto').attr('disabled','disabled');
                   // ocultar boton agregar
                   $('#add_product_venta').slideDown();              //aqui si quiero agregar mas productos directamente
               }else{
                console.log('no data');
               }
               viewProcesar();
           ...
           if (response != 'error')
            {
                ...
                //$('.nameProducto').html(info.descripcion);
                 ...
                 $('.bodyModal').html('<form action=""method="post"name="form_anular_factura" id="form_anular_factura" onsubmit="event.preventDefault(); anularFactura();">'+
                                     '<h1><br>Anular Factura</h1>'+
                                      '<p>¿Esta seguro de anular la factura?</p>'+
                                     ...
                                     '<div class="alert alertAddProduct"></div>'+
                                     '<button  type="submit" class="btn_new">Anular</button>'+
                                     '<a href="#" class="btn_ok" onclick="coloseModal();">Cerrar</a>'+
                                     '</form>');               
            }
            },
...
if (passNuevo != confirmarPassNuevo)
  ...
  if (passNuevo.length < 12){
    $('.alertChangePass').html('<p style="color:red">La contraseña debe tener como minimo 12 caracteres.</p>')
    $('.alertChangePass').slideDown();
    return false;
  }
...
function medirPass()
{
    var medirNuevo =$('#txtNewPassUser').val();
    var medirPassNuevo =$('#txtPassConfirm').val(); 
    if (medirNuevo.length > 1 && medirNuevo.length <= 4 ){
      $('.alertMedPass').html('<p style="color:red">Nivel de contraseña baja.</p>')
      $('.alertMedPass').slideDown();
      return false;

      } else if (medirNuevo.length > 4 && medirNuevo.length <=8 ){
        $('.alertMedPass').html('<p style="color:#cad606">Nivel de contraseña medio.</p>')
        $('.alertMedPass').slideDown();
        return false;
      }else if (medirNuevo.length > 8 ){
        $('.alertMedPass').html('<p style="color:green">Nivel de contraseña alto.</p>')
        $('.alertMedPass').slideDown();
        return false;
      }
}
              ...
              if (response == 'error')
               {
                ...
                $('#form_anular_factura .btn_new').remove();
...
function sendDataProduct()
{
    ...
    $.ajax({
            ...
            if (response == 'error')
             {
                $('.alertAddProduct').html('<p style="color:red;">Error al agregar peoducto</p>');
             }else
             {
...
function delProduct()
{
    ...
    $.ajax({
            ...
            if (response == 'error')
             {
                $('.alertAddProduct').html('<p style="color:red;">Error al eliminar peoducto</p>');
             }else
             {
